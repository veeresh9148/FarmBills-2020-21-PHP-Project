 <?php
    ...
    function getmarketname()
    {
        if (isset($_SESSION['phonenumber'])) {
            ...
            $query = "select * from marketregistration where market_phone = $phonenumber";
            $run_query = mysqli_query($con, $query);
            if ($run_query) {
                while ($row_cat = mysqli_fetch_array($run_query)) {
                    $market_name = $row_cat['market_name'];
                    $market_name = 'Hello ,' . $market_name;
                }

                // echo @"<label>$market_name</label>";
                echo @"<div class='text-success  logins mx-1 ml-5  '>$market_name</div>";
            }
        } else {
            echo "<a href = '../auth/MarketLogin.php'><div class='text-success logins mx-5'>Login</div></a>";
            // echo "<label><a href = '../auth/MarketLogin.php' style = 'color:white' >Login/Sign up</a></label>";
        }
    }
    function getusername1()
    {
        if (isset($_SESSION['username'])) {
            ...
            echo "<a href = '../Government/GovernmentLogin.php'><div class='text-success logins mx-5'>Login</div></a>";
            // echo "<label><a href = '../Government/login/login.php' style = 'color:white' >Login/Sign up</a></label>";
             }
    }
    ...
    function getbankUsername()
    {
        if (isset($_SESSION['phonenumber'])) {
            $phonenumber = $_SESSION['phonenumber'];
            global $con;

            $query = "select * from bankregistration where bank_phone = $phonenumber";
            $run_query = mysqli_query($con, $query);
            if ($run_query) {
                while ($row_cat = mysqli_fetch_array($run_query)) {
                    $buyer_name = $row_cat['bank_name'];
                    $buyer_name = "Hello ," . $buyer_name;
                    echo "<label style = 'color:white; padding-top:7px;'>$buyer_name</label>";
                }
            }
        } else {
            echo "<label><a href = '../auth/bankLogin.php' style = 'color:white; padding-top:20px;' >Login/Sign up</a></label>";
        }
    }
